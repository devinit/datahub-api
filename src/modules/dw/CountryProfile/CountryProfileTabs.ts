...
import { IGetIndicatorArgs, IRAW, IRAWPopulationAgeBand, IRAWPopulationGroup, IRAWQuintile } from '../../utils/types';
import {
  getIndicatorData, getIndicatorToolTip, getIndicatorsValue,
  indicatorDataProcessingNamed, isDonor
} from '../../utils';
...
export default class CountryProfileTabs {
  ...
  public async getOverViewTab({ id }): Promise<DH.IOverviewTab> {
    try {
      ...
      const [ governmentSpendPerPerson ] = await getIndicatorsValue({
        id, format: false,
  ...
  public async getPovertyTab({ id }): Promise<DH.IPovertyTab> {
    try {
      const isDonorCountry = await isDonor(id);
      if (isDonorCountry) {
        return { poverty190Trend: null, depthOfExtremePoverty: null, incomeDistTrend: null };
      }

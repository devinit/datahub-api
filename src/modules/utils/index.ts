import { approximate, getTableNameFromSql, isError, toId, toNumericFields } from '@devinit/prelude';
...
import {
  ...
  getCreditorType,
  getCurrency,
  getDestinationInstitutionType,
  getEntities,
  getEntityByIdGeneric,
  getEntityBySlugAsync,
  getFinancingType,
  getFlowType,
  getSectors
} from '../refs/global';
...
import {
  IGetIndicatorArgs,
  IGetIndicatorArgsSimple,
  IGetIndicatorValueArgs,
  IMissingDomesticParent,
  IProcessedSimple,
  IRAW,
  IRAWDomestic,
  ISpotlightGetIndicatorArgs,
  IToolTipArgs,
  IhasDiId
} from './types';
...
export const getSpotlightTableName = (country: string, query: string): string => {
  ...
  if (isError(tableStr)) {
    throw new Error(`error getting table name for : ${query}`);
  }
...
export async function getIndicatorData<T>(opts: IGetIndicatorArgs): Promise<T[]> {
  ...
  const tableName = table || getTableNameFromSql(query);
  if (isError(tableName)) {
    throw Error(`error getting table name: ${query}`);
  }
  let countryEntity: any = {};
  if (conceptType === 'country-profile' && id) {
    countryEntity = await getEntityBySlugAsync(id);
  }
...
export async function getIndicatorDataSpotlights<T>(opts: ISpotlightGetIndicatorArgs): Promise<T[]> {
  ...
  if (!tableName) {
    throw new Error('Provide a valid table name or query string');
  }
...
export const getIndicatorsValue = async ({ id, sqlList, db, format = true, precision }: IGetIndicatorValueArgs)
  ...
  try {
    const indicatorArgs: IGetIndicatorArgs[] = sqlList.map(query =>
      ({ db, conceptType: 'country-profile', query, id }));
    ...
    return indicatorRaw.map((data, index) => {
      ...
      let value = 'No data';
      if (data && data[0] && data[0].value && format) {
        value = approximate(data[0].value, precisionFix, true);
      }
      if (data && data[0] && data[0].value && !format) {
        value = Math.round(Number(data[0].value)).toString();
      }
...
export const indicatorDataProcessingSimple = <T extends {}>(data: any, country?: string): T[] => {
  return data
    .map(toId)
    .map((obj) => country === 'global' || !country ? obj : { ...obj, id: obj.district_id })
...
export const addColorToDomesticLevels =
  (levels: string[], budgetRefs: IBudgetLevelRef[], colors: IColor[]): string => {
    const levelRefs = levels.map(level => budgetRefs.find(ref => ref.name === level)) as IBudgetLevelRef[];
    const colorObjs: IColor[] = levelRefs.map(levelRef => {
      const levelRefColor = levelRef && levelRef.color ? levelRef.color : 'blue-light';

      return getEntityByIdGeneric(levelRefColor, colors);
    });

    return levels.length < 2 ? colorObjs[0].value : colorObjs[1].value;
  };

export const domesticDataProcessing = async (data: IRAWDomestic[], country?: string): Promise<DH.IDomestic[]> => {
  ...
  return indicatorDataProcessingSimple(data).map((obj: any) => { // TODO: make type for IRAWDomestic processed
    const levelKeys: string[] = R.keys(obj).filter(key => R.test(/^l[0-9]/, key));
    if (!levelKeys) {
      throw new Error(`Levels missing in budget data ${JSON.stringify(obj)}`);
    }
...
export const isDonor = async (slug: string): Promise<boolean> => {
  ...
  return obj.donor_recipient_type === DONOR;
};
...
export const makeSqlAggregateQuery = (queryArgs: any, groupByField: string, table: string): string => {
  ...
  return queryArgsKeys.reduce((query, field, index) => {
    ...
    return field === 'year'
      ? `${query} ${field} = ${queryArgs[field]} ${AND}`

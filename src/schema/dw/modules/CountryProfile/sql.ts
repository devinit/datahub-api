export default {
    ...
    ODANetIn: 'SELECT value FROM fact.in_oda_net_2015 WHERE di_id = ${id} AND year = ${startYear}',
    PercentODANetOut: 'SELECT value FROM fact.in_oda_net_2015 WHERE di_id = ${id} AND year = ${startYear}',
    // tslint:disable-next-line:max-line-length
    resourcesDonors: 'SELECT value FROM data_series.intl_flows_donors WHERE di_id = ${id} AND year = ${startYear} AND direction = \'out\'',
    // tslint:disable-next-line:max-line-length
